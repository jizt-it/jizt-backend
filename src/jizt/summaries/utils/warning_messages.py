...
class WarningMessage(Enum):
    ...
    # int value
    INT = "Value must be an int. Using default value instead."
    # float value
    FLOAT = "Value must be a float. Using default value instead."
    # numeric value with a lower bound (inclusive)
    LOWER_BOUNDED = ("Value must be greater or equal to {lower_bound}. Using "
                     "default value instead.")
    # numeric value with lower and upper bounds (inclusive)
    BOUNDED = ("Value must be in the range [{lower_bound}, {upper_bound}]. "
               "Using default values instead.")
    # bool value
    BOOL = "Value must be a bool. Using default value instead."
    # unsupported parameter
    UNSUPPORTED = "Parameter not supported. It will be ignored."
    # relative_max_length
    MAX_LENGTH = ("Value must be greater than 'relative_min_length'. Using "
                  "default value instead.")
    # relative_min_length
    MIN_LENGTH = ("Value must be less than 'relative_max_length'. Using default "
                  "value instead.")
    # default relative_max_length
    MAX_LENGTH_DEFAULT = (f"Value must be greater than the default "
                          f"'relative_min_length' "
                          f"({SUMM_DEFAULTS.model_params.relative_min_length}). "
                          f"Using default value instead.")
    # default relative_min_length
    MIN_LENGTH_DEFAULT = (f"Value must be lower than the default "
                          f"'relative_max_length' "
                          f"({SUMM_DEFAULTS.model_params.relative_max_length}). "
                          f" Using default value instead.")
    ...
    def __call__(
        ...
        **kwargs: dict
        ...
        Args:
            ...
            **kwargs (:obj:`dict`):
                Additional info, the bounds of the parameter in case of it being
                a numerical value.
    ...
    def _build_message(
        ...
        Args:
            ...
            lower_bound (:obj:`int` or :obj:`float`, `optional`, defaults to :obj:`None`):
                The lower bound of the parameter (if any).
            upper_bound (:obj:`int` or :obj:`float`, `optional`, defaults to :obj:`None`):
                The upper bound of the parameter (if any).
